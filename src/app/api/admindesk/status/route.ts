  ...
  try {
    ...
    if (process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY) {
      ...
      try {
        ...
        if (meetErr) {
          ...
          // Query participants details & messages count safely
          const [participantsRes, messagesRes] = await Promise.all([
            (async () => {
              try {
                return await supabase.from('meeting_participants').select('id, meeting_id, name, joined_at, last_seen_at', { count: 'exact' });
              } catch {
            ...
            (async () => {
              try {
                return await supabase.from('meeting_messages').select('id', { count: 'exact', head: true });
              } catch {
    ...
    return NextResponse.json({
      ...
      livekitRooms: livekitRooms.map((r) => ({
        ...
        creationTime: (() => {
          try {
            if (!r.creationTime) return null;
            const num = Number(r.creationTime);
    ...
    return NextResponse.json(
      {
        ...
        checks: [
          {
            name: 'System Status Engine',
            category: 'engine',
            status: 'down',
            latencyMs: Date.now() - startTime,
            message: `Status check failed: ${err?.message || 'Unknown error'}`,
          },
        ],
        limits: [],
        metrics: {
